...
static void broadcast_memory_status(const char *label) {
    ...
    ESP_LOGI("WEBSERVER", "=== Memory %s - PSRAM: %" PRIu32, label, psram);  // Keep log
...
//    cJSON_AddStringToObject(obj, "project_name", desc->project_name);
    cJSON_AddStringToObject(obj, "idf_version",  desc->idf_ver);
//    cJSON_AddStringToObject(obj, "build_date",   desc->date);
//    cJSON_AddStringToObject(obj, "build_time",   desc->time);
